...
Hierarchy::Hierarchy(const std::shared_ptr<ICE::ICEEngine> &engine) : m_engine(engine) {
    ...
    ui.registerCallback("create_entity_clicked", [this](ICE::Entity parent) {
        ...
        auto cube_id = m_engine->getAssetBank()->getUID(ICE::AssetPath::WithTypePrefix<ICE::Model>("cube"));
        scene->getRegistry()->addComponent<ICE::RenderComponent>(entity, ICE::RenderComponent(cube_id));
