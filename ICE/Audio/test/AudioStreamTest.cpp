...
TEST(AudioStreamTest, ReportsFormatFromTheHeader) {
    fs::path wav = writeRampWav("ice_stream_fmt.wav", 1000, 2, 22050);
    auto stream = OpenAudioFileStream(wav);
...
TEST(AudioStreamTest, ReadsIncrementallyInOrder) {
    fs::path wav = writeRampWav("ice_stream_seq.wav", 500);
    auto stream = OpenAudioFileStream(wav);
    ASSERT_NE(stream, nullptr);
...
TEST(AudioStreamTest, ShortReadAtEndThenZero) {
    fs::path wav = writeRampWav("ice_stream_eof.wav", 150);
    auto stream = OpenAudioFileStream(wav);
    ASSERT_NE(stream, nullptr);
...
TEST(AudioStreamTest, RewindReturnsToTheStart) {
    fs::path wav = writeRampWav("ice_stream_rewind.wav", 200);
    auto stream = OpenAudioFileStream(wav);
    ASSERT_NE(stream, nullptr);
...
TEST(AudioStreamTest, LoopFillAcrossTheEndHasNoGap) {
    ...
    fs::path wav = writeRampWav("ice_stream_loop.wav", total);
    auto stream = OpenAudioFileStream(wav);
    ASSERT_NE(stream, nullptr);
...
TEST(AudioStreamTest, LoaderStreamsFilesAtOrAboveTheThreshold) {
    fs::path big = writeRampWav("ice_stream_big.wav", 40000);  // ~80 KB of PCM
