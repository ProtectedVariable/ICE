...
bool AssetViewPane::render() {
    ...
    if (engine->getAssetBank()->getAll<Texture>().find(*selectedAsset) == engine->getAssetBank()->getAll<Texture>().end()) {
        AssetUID previewMeshId = engine->getAssetBank()->getAll<Mesh>().find(*selectedAsset) == engine->getAssetBank()->getAll<Mesh>().end()
            ? engine->getAssetBank()->getUID(AssetPath::WithTypePrefix<Mesh>("__ice__sphere"))
            : *selectedAsset;
        AssetUID mat = engine->getAssetBank()->getAll<Material>().find(*selectedAsset) == engine->getAssetBank()->getAll<Material>().end()
        ...
        auto sphere = Entity();
        auto rcSphere = RenderComponent{.mesh = previewMeshId, .material = mat, .shader = engine->getAssetBank()->getUID(AssetPath::WithTypePrefix<Shader>("__ice__phong_shader"))};
...
AssetViewPane::AssetViewPane(ICEEngine* engine, AssetUID* selectedAsset)
    : engine(engine),
